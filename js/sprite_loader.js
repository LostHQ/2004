...
async function drawNPCImage(ctx, x, y, npcId, iconSize) {
    ...
    if (npcBitmap) {
        if (sx + npcSpriteSize > npcBitmapWidth || sy + npcSpriteSize > npcBitmapHeight) return;
...
const stackableSpriteOverrides = {
    ...
    887: [
        ...
        [4, 914],
        [5, 917],
    ], // Steel arrow(p) (steel_arrow_p)
    888: [
        [2, 920],
        [3, 919],
        [4, 918],
        [5, 921],
    ], // Mithril arrow (mithril_arrow)
    889: [
        [2, 924],
        [3, 923],
        [4, 922],
        [5, 925],
    ], // Mithril arrow(p) (mithril_arrow_p)
    890: [
        [2, 928],
        [3, 927],
        [4, 926],
        [5, 929],
    ], // Adamant arrow (adamant_arrow)
    891: [
        [2, 932],
        [3, 931],
        [4, 930],
        [5, 933],
    ], // Adamant arrow(p) (adamant_arrow_p)
    892: [
        [2, 936],
        [3, 935],
        [4, 934],
        [5, 937],
    ], // Rune arrow (rune_arrow)
    893: [
        [2, 940],
        [3, 939],
        [4, 938],
        [5, 941],
    ], // Rune arrow(p) (rune_arrow_p)
...
function formatQuantity(quantity) {
    ...
    if (quantity > 2147483647) return "2147M";
    if (quantity > 9999999) return `${Math.floor(quantity / 1000000)}M`;
    if (quantity > 99999) return `${Math.floor(quantity / 1000)}K`;
...
function renderNPCSpriteToCanvas(canvas) {
    ...
    const iconSize = parseInt(canvas.getAttribute("icon-size"), 12) || npcSpriteSize;
    let id = 0;
...
function renderItemSpriteToCanvas(canvas) {
    ...
    const requestedSize = parseInt(canvas.getAttribute("icon-size")) || itemSpriteSize;
    const iconSize = requestedSize !== itemSpriteSize && qty > 1 ? itemSpriteSize : requestedSize;
